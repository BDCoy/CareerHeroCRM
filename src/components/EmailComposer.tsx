...
import { EmailTemplate, getEmailTemplates, processTemplate } from '../lib/emailService';
...
interface EmailComposerProps {
  ...
  customerName: string;
  onSendEmail: (to: string, subject: string, body: string, cc?: string[], bcc?: string[]) => Promise<void>;
...
const EmailComposer: React.FC<EmailComposerProps> = ({
  ...
  const { 
    ...
    defaultValues: {
      ...
      cc: '',
      bcc: '',
      subject: replyToEmail ? `Re: ${parseEmailContent(replyToEmail.content).subject}` : '',
  ...
  const onSubmit = async (data: EmailFormData) => {
    ...
    try {
      ...
      const cc = data.cc ? data.cc.split(',').map(email => email.trim()) : undefined;
      const bcc = data.bcc ? data.bcc.split(',').map(email => email.trim()) : undefined;
      
      await onSendEmail(data.to, data.subject, data.body, cc, bcc);
            ...
            {attachments.map((file, index) => (
              <li key={index} className="flex items-center justify-between text-sm">
                <div className="flex items-center">
                  {getFileIcon(file)}
                  <span className="ml-2 truncate max-w-[150px] sm:max-w-full">{file.name}</span>
