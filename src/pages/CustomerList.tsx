...
import { Search, UserPlus, Edit, Trash2, Filter, Phone, Mail, MessageSquare, CheckSquare, Square, X, Send, AlertCircle } from 'lucide-react';
...
const CustomerList: React.FC = () => {
  ...
  const [bulkAction, setBulkAction] = useState<'email' | 'sms' | 'whatsapp' | null>(null);
  ...
  // Start bulk action
  const startBulkAction = (action: 'email' | 'sms' | 'whatsapp') => {
        ...
        <div className="flex flex-wrap gap-2">
          {bulkMode ? (
            <>
              ...
              <button
                onClick={() => startBulkAction('sms')}
                disabled={selectedCustomers.length === 0}
                className="inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 disabled:opacity-50"
              >
                <MessageSquare className="h-4 w-4 mr-2" />
                Bulk SMS ({selectedCustomers.length})
              </button>
              <button
